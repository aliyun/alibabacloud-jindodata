...
执行以下命令，下载6.8.3版本JindoSDK JAR包。以大多数linux x86环境为例。其他平台部署参见[《在多平台环境安装部署 JindoSDK》](jindosdk_deployment_multi_platform.md)
...
以 jindosdk-6.8.3 为例:
```
cp indo-core-6.8.3.jar <HADOOP_HOME>/share/hadoop/hdfs/lib/
cp indosdk-6.8.3.jar <HADOOP_HOME>/share/hadoop/hdfs/lib/
```
