...
安装方式的话，即可以通过maven在pom.xml中配置依赖，如[6.4.0-oss-maven](oss-maven.md)。也可以通过oss地址直接下载部署，如[6.4.0-download](jindosdk_download.md)。
...
1. 完整安装，以安装包内容解压在 /usr/lib/jindosdk-6.4.0-linux 目录为例：
...
1. 完整安装，以安装包内容解压在 /usr/lib/jindosdk-6.4.0-linux-el7-aarch64 目录为例：
