...
以将新版集群中的 JindoSDK 升级到 6.0.0 版本，linux x86 平台为例:
...
-rw-r----- 1 emr-user emr-user xxxxxxxxx May 01 00:00 jindosdk-6.0.0-linux.tar.gz
```
...
以将新版集群中的 JindoSDK 从 4.6.5 版本升级到 6.0.0 版本为例:

```bash
./apply_all.sh 6.0.0 4.6.5
```
...
以将新版集群中的 JindoSDK 升级到 6.0.0 版本，linux x86平台为例:
...
```bash
bash bootstrap_jindosdk.sh -gen 6.0.0 4.6.5
```
