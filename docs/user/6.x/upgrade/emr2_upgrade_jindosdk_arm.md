...
以将新版集群中的 JindoSDK 升级到 6.10.7 版本，linux arm 平台为例:

```bash
cd jindosdk-patches

wget https://jindodata-binary.oss-cn-shanghai.aliyuncs.com/release/6.10.7/jindosdk-6.10.7-linux-el7-aarch64.tar.gz
...
-rw-r----- 1 emr-user emr-user xxxxxxxxx May 01 00:00 jindosdk-6.10.7-linux-el7-aarch64.tar.gz
```
...
以将新版集群中的 JindoSDK 升级到 6.10.7 版本为例:
...
以从集群默认版本 6.2.0 升级为 6.10.7 版本为例，返回示例如下：
```bash
...
以将新版集群中的 JindoSDK 升级到 6.10.7 版本，linux aarch64平台为例:
...
-rw-r----- 1 hadoop hadoop xxxxxxxxx May 01 00:00 jindosdk-6.10.7-linux-el7-aarch64.tar.gz
...
```bash
bash bootstrap_jindosdk.sh -gen 6.10.7
```
