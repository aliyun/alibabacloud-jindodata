...
该 jar 包含在 jindosdk-${version}.tar.gz ([下载页面](/docs/user/jindosdk/jindosdk_download.md))，解压缩后可在 plugins/flink/ 目录下找到。
...
注：JindoSDK 目前提供多平台支持，当前支持的平台以及需要额外放置的资源，参考 [在多平台环境安装部署 JindoSDK](/docs/user/jindosdk/jindosdk_deployment_multi_platform.md)
