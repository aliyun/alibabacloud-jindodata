...
以将新版集群中的 jindocache 升级到 6.9.1 版本，linux x86 平台为例:

```bash
cd jindocache-patches

wget https://jindodata-binary.oss-cn-shanghai.aliyuncs.com/release/6.9.1/jindocache-6.9.1-linux.tar.gz

tar -zxvf jindocache-6.9.1-linux.tar.gz
...
drwxr-xr-x  7 emr-user emr-user          4096 8月   8 16:19 jindocache-6.9.1-linux
-rw-r--r--  1 emr-user emr-user     276406433 8月  12 14:32 jindocache-6.9.1-linux.tar.gz
...
以将新版集群中的 jindocache 升级到 linux 平台 6.9.1 版本为例:

```bash
bash upgrade.sh 6.9.1 linux
```

脚本执行完成后，返回如下提示信息。

```
Start upgrade JindoCache Service to 6.9.1 with platform linux
...
Successfully upgrade JindoCache Service to 6.9.1 with platform linux, Please restart JindoCache Service
```
...
以将新版集群中的 jindocache 升级到 6.9.1 版本，linux x86平台为例:
...
-rw-r----- 1 hadoop hadoop xxxxxxxxx May 01 00:00 jindocache-6.9.1-linux.tar.gz
...
以将新版集群中的 jindocache 升级到 6.9.1 版本为例:

```bash
bash bootstrap_jindocache.sh -gen 6.9.1
```
