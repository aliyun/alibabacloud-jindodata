...
以将新版集群中的 jindocache 升级到 6.4.0 版本，linux x86 平台为例:

```bash
cd jindocache-patches

wget https://jindodata-binary.oss-cn-shanghai.aliyuncs.com/release/6.4.0/jindocache-6.4.0-linux.tar.gz
...
以将新版集群中的 jindocache 升级到 linux 平台 6.4.0 版本为例:

```bash
bash upgrade.sh 6.4.0 linux
```

脚本执行完成后，返回如下提示信息。

```
Start upgrade JindoCache Service to 6.4.0 with platform linux
...
Successfully upgrade JindoCache Service to 6.4.0 with platform linux, Please restart JindoCache Service
```
