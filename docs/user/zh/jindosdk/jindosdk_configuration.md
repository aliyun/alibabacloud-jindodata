...
| fs.oss.append.threshold.size               |  整型  |  0         | 客户端append写入时会检查最后一个block是否小于`fs.oss.append.threshold.size`。若小于，则从最后一个block末尾追加写；否则新建一个block开始写。若上一个block是虚拟块，则新建块。    | 6.7.6+ nextarch    |
| fs.oss.flush.merge.threshold.size               |  整型  |  1048576         | 打开后，flush时虚拟块会合并小于`fs.oss.flush.merge.threshold.size`的slice。 | 6.7.6+ nextarch    |
