...
安装方式的话，即可以通过maven在pom.xml中配置依赖，如[6.6.1-oss-maven](oss-maven.md)。也可以通过oss地址直接下载部署，如[6.6.1-download](jindosdk_download.md)。
...
ps: 6.6.1 版本开始 tar 包中的 lite jar 默认使用 nextarch classifer，即 jindo-core-x.y.z-nextarch.jar、jindo-core-linux-el7-aarch64-x.y.z-nextarch.jar 。。
...
1. 完整安装，以安装包内容解压在 /usr/lib/jindosdk-6.6.1-linux 目录为例：
...
1. 完整安装，以安装包内容解压在 /usr/lib/jindosdk-6.6.1-linux-el7-aarch64 目录为例：
