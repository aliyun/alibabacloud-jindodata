...
上面的`fs.fsx.oss.accessKeyId`和`fs.fsx.oss.accessKeySecret`将会明文配置在`core-site.xml`中，可以将其以加密对方式存储至 Hadoop Credential Providers文件中。<br />
...
```
hadoop credential create fs.fsx.oss.accessKeyId -value AAA -provider jceks://file/root/oss.jceks
hadoop credential create fs.fsx.oss.accessKeySecret -value BBB -provider jceks://file/root/oss.jceks
hadoop credential create fs.fsx.oss.securityToken -value  CCC -provider jceks://file/root/oss.jceks
```
生成Credential文件后，您需要配置下面的参数来指定Provider的类型和位置。
```xml
<configuration>
    <property>
        <name>fs.fsx.oss.security.credential.provider.path</name>
...
<configuration>
    <property>
        <name>fs.fsx.oss.credentials.provider</name>
...
<configuration>
    ...
    <property>
        <name>fs.fsx.oss.securityToken</name>
