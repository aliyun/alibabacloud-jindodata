...
客户端本地缓存功能通过插件形式提供动态库，需要配置路径以加载动态库，动态库在jindosdk-4.6.11/plugins/路径下，将该路径的绝对路径添加到 Hadoop 的`core-site.xml`中。
```xml
<configuration>
    <property>
...
以配置文件`jindosdk.cfg`在`/usr/lib/jindosdk-4.6.11/conf`目录为例：
```bash
export JINDOSDK_CONF_DIR=/usr/lib/jindosdk-4.6.11/conf
```
...
# 客户端本地缓存功能通过插件形式提供动态库，需要配置路径以加载动态库，动态库在jindosdk-4.6.11/plugins/路径下
