...
wget https://jindodata-binary.oss-cn-shanghai.aliyuncs.com/release/4.6.5/jindosdk-4.6.5.tar.gz
...
-rw-r----- 1 emr-user emr-user xxxxxxxxx May 01 00:00 jindosdk-4.6.5.tar.gz
```
...
下载的 jindosdk-patches.tar.gz ，jindosdk-4.6.5.tar.gz 和 [bootstrap_jindosdk.sh](https://jindodata-binary.oss-cn-shanghai.aliyuncs.com/resources/emr-taihao/bootstrap_jindosdk.sh),
...
-rw-r----- 1 hadoop hadoop xxxxxxxxx May 01 00:00 jindosdk-4.6.5.tar.gz
-rw-r----- 1 hadoop hadoop      xxxx May 01 00:00 jindosdk-patches.tar.gz
```

执行命令制作升级包

支持三个参数：`-gen` 包含完整的 JindoSDK 功能; `-gen-lite` 仅包含 HadoopSDK 功能，支持 EMR on X86 架构; `-gen-lite-with-arm` 包含 HadoopSDK 功能，并支持 EMR on X86/倚天arm架构
