...
-rw-r----- 1 emr-user emr-user xxxxxxxxx May 01 00:00 jindosdk-4.6.12-linux.tar.gz
```
...
下载的 jindosdk-patches.tar.gz ，jindosdk-4.6.12-linux.tar.gz 和 [bootstrap_jindosdk.sh](https://jindodata-binary.oss-cn-shanghai.aliyuncs.com/resources/emr-taihao/bootstrap_jindosdk.sh),
...
```bash
bash bootstrap_jindosdk.sh -gen 4.6.12 4.6.5
```
...
```bash
bash bootstrap_jindosdk.sh -gen-lite 4.6.12 4.6.5
```
