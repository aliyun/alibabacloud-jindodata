...
| 参数名称         | 必选 | 参数说明                                                     | 使用范例             |
| ---------------- | ---- | ------------------------------------------------------------ | -------------------- |
| uri              | ✓    | 配置需要映射的 oss 路径。路径可以是根目录，也可以是子目录。例如：oss://bucket.bucket/ 或 oss://bucket.endpoint/subdir。 | -ouri=oss://bucket.endpoint/，如 -ouri=oss://test-bucket.cn-beijing.oss-dls.aliyuncs.com/  |
