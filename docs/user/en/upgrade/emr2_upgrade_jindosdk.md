...
Download the JindoSDK software package `jindosdk-{VERSION}-{PLATFORM}.tar.gz`, replacing `{VERSION}` and `{PLATFORM}` appropriately (for example, `6.7.2` for version and `linux` for platform).
...
For instance, to upgrade to version `6.7.2`, run:
...
For example, upgrading to version `6.7.2` on Linux x86:
...
For example, to upgrade the new cluster's JindoSDK to version `6.7.2` on Linux x86:
...
For upgrading to version `6.7.2`, run:
