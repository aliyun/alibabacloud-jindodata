...
Download the JindoSDK software package `jindosdk-{VERSION}-{PLATFORM}.tar.gz`, replacing `{VERSION}` and `{PLATFORM}` appropriately (for example, `6.10.3` for version and `linux` for platform).
...
For instance, to upgrade to version `6.10.3`, run:
...
For example, upgrading to version `6.10.3` on Linux x86:
...
For version `6.10.3`, run:

```bash
bash bootstrap_jindosdk.sh -gen-full 6.10.3
```
...
```bash
lrwxrwxrwx 1 root root 64 Apr 12 11:08 jindo-core-6.2.0.jar -> /opt/apps/JINDOSDK/jindosdk-6.10.3-linux/lib/jindo-core-6.10.3.jar
lrwxrwxrwx 1 root root 82 Apr 12 11:08 jindo-core-linux-el7-aarch64-6.2.0.jar -> /opt/apps/JINDOSDK/jindosdk-6.10.3-linux/lib/jindo-core-linux-el7-aarch64-6.10.3.jar
lrwxrwxrwx 1 root root 63 Apr 12 11:08 jindo-sdk-6.2.0.jar -> /opt/apps/JINDOSDK/jindosdk-6.10.3-linux/lib/jindo-sdk-6.10.3.jar
lrwxrwxrwx 1 root root 50 Apr 12 11:08 native -> /opt/apps/JINDOSDK/jindosdk-6.10.3-linux/lib/native
lrwxrwxrwx 1 root root 57 Apr 12 11:08 site-packages -> /opt/apps/JINDOSDK/jindosdk-6.10.3-linux/lib/site-packages
```
...
For example, to upgrade the new cluster's JindoSDK to version `6.10.3` on Linux x86:
...
-rw-r----- 1 hadoop hadoop xxxxxxxxx May 01 00:00 jindosdk-6.10.3-linux.tar.gz
...
For upgrading to version `6.10.3`, run:
