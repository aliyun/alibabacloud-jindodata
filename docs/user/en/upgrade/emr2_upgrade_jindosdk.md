...
Download the JindoSDK software package `jindosdk-{VERSION}-{PLATFORM}.tar.gz`, replacing `{VERSION}` and `{PLATFORM}` appropriately (for example, `6.10.2` for version and `linux` for platform).
...
For instance, to upgrade to version `6.10.2`, run:
...
For example, upgrading to version `6.10.2` on Linux x86:
...
For example, to upgrade the new cluster's JindoSDK to version `6.10.2` on Linux x86:
...
-rw-r----- 1 hadoop hadoop xxxxxxxxx May 01 00:00 jindosdk-6.10.2-linux.tar.gz
...
For upgrading to version `6.10.2`, run:
