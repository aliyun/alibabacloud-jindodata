...
Download the JindoSDK software package `jindosdk-{VERSION}-{PLATFORM}.tar.gz`, replacing `{VERSION}` and `{PLATFORM}` appropriately (for example, `6.3.4` for version and `linux` for platform).
...
For instance, to upgrade to version `6.3.4`, run:
...
For example, upgrading to version `6.3.4` on Linux x86:
...
For example, to upgrade the new cluster's JindoSDK to version `6.3.4` on Linux x86:
