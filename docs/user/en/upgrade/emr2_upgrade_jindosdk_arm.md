...
For upgrading the JindoSDK in a new cluster version to `6.7.6` on a Linux ARM environment:

```bash
cd jindosdk-patches
wget https://jindodata-binary.oss-cn-shanghai.aliyuncs.com/release/6.7.6/jindosdk-6.7.6-linux-el7-aarch64.tar.gz
ls -l
```
...
-rw-r----- 1 emr-user emr-user xxxxxxxxx May 01 00:00 jindosdk-6.7.6-linux-el7-aarch64.tar.gz
```
...
For upgrading to version `6.7.6`, run:
...
-rw-r----- 1 hadoop hadoop xxxxxxxxx May 01 00:00 jindosdk-6.7.6-linux-el7-aarch64.tar.gz
