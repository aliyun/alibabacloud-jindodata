...
For upgrading the JindoSDK in a new cluster version to `6.3.3` on a Linux ARM environment:

```bash
cd jindosdk-patches
wget https://jindodata-binary.oss-cn-shanghai.aliyuncs.com/release/6.3.3/jindosdk-6.3.3-linux-el7-aarch64.tar.gz
ls -l
```
...
-rw-r----- 1 emr-user emr-user xxxxxxxxx May 01 00:00 jindosdk-6.3.3-linux-el7-aarch64.tar.gz
```
...
For upgrading to version `6.3.3`, run:
...
-rw-r----- 1 hadoop hadoop xxxxxxxxx May 01 00:00 jindosdk-6.3.3-linux-el7-aarch64.tar.gz
...
```bash
bash bootstrap_jindosdk.sh -gen-full 6.3.3
```
**Explanation of parameters: `-gen` generates a lite upgrade package, `-gen-full` indicates generating a full upgrade package.**
