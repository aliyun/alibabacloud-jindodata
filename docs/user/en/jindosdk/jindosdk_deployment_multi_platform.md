...
Installation can be done through Maven by adding dependencies in `pom.xml` (refer to [6.7.3-OSS-Maven](oss-maven.md)) or directly downloading and deploying from OSS (see [6.7.3-Download](jindosdk_download.md)).
...
Let's take version 6.7.3 as an example.

## Linux x86 Example

### Full Installation
Assuming the extracted folder is placed at `/usr/lib/jindosdk-6.7.3-linux`:

```bash
wget https://jindodata-binary.oss-cn-shanghai.aliyuncs.com/release/6.7.3/jindosdk-6.7.3-linux.tar.gz
tar zxvf jindosdk-6.7.3-linux.tar.gz -C /usr/lib/
export JINDOSDK_HOME=/usr/lib/jindosdk-6.7.3-linux
...
cp jindo-core-6.7.3.jar <HADOOP_HOME>/share/hadoop/hdfs/lib/
cp jindosdk-6.7.3.jar <HADOOP_HOME>/share/hadoop/hdfs/lib/
```

## Linux aarch64 Example

### Full Installation
Assuming the extracted folder is placed at `/usr/lib/jindosdk-6.7.3-linux-el7-aarch64`:

```bash
wget https://jindodata-binary.oss-cn-shanghai.aliyuncs.com/release/x.y.z/jindosdk-6.7.3-linux-el7-aarch64.tar.gz
tar zxvf jindosdk-6.7.3-linux-el7-aarch64.tar.gz -C /usr/lib/
export JINDOSDK_HOME=/usr/lib/jindosdk-6.7.3-linux-el7-aarch64
export JINDOSDK_CONF_DIR=${JINDOSDK_HOME}/conf
export PATH=${PATH}:${JINDOSDK_HOME}/bin
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:${JINDOSDK_HOME}/lib/native
export HADOOP_CLASSPATH=$HADOOP_CLASSPATH:${JINDOSDK_HOME}/lib/*
...
```bash
wget https://jindodata-binary.oss-cn-shanghai.aliyuncs.com/mvn-repo/com/aliyun/jindodata/jindo-sdk/6.7.3/jindo-sdk-6.7.3.jar
wget https://jindodata-binary.oss-cn-shanghai.aliyuncs.com/mvn-repo/com/aliyun/jindodata/jindo-core-linux-el7-aarch64/6.7.3/jindo-core-linux-el7-aarch64-6.7.3.jar
cp jindo-core-linux-el7-aarch64-6.7.3.jar <HADOOP_HOME>/share/hadoop/hdfs/lib/
cp jindo-core-6.7.3.jar <HADOOP_HOME>/share/hadoop/hdfs/lib/
cp jindosdk-6.7.3.jar <HADOOP_HOME>/share/hadoop/hdfs/lib/
```
