...
Execute the following commands to download the v6.10.5 JindoSDK JAR packages for most Linux x86 systems. For other platform deployments, refer to [Deploying JindoSDK on Multi-platform Environments](jindosdk_deployment_multi_platform.md):
```bash
wget https://jindodata-binary.oss-cn-shanghai.aliyuncs.com/mvn-repo/com/aliyun/jindodata/jindo-sdk/6.10.5/jindo-sdk-6.10.5.jar
wget https://jindodata-binary.oss-cn-shanghai.aliyuncs.com/mvn-repo/com/aliyun/jindodata/jindo-core/6.10.5/jindo-core-6.10.5.jar
```
...
```bash
cp jindo-core-6.10.5.jar <HADOOP_HOME>/share/hadoop/hdfs/lib/
cp jindo-sdk-6.10.5.jar <HADOOP_HOME>/share/hadoop/hdfs/lib/
```
