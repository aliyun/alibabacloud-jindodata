...
以将新版集群中的 JindoSDK 升级到 5.0.0-beta1 版本，linux x86 平台为例:

```bash
cd jindosdk-patches

wget https://jindodata-binary.oss-cn-shanghai.aliyuncs.com/release/5.0.0-beta1/jindosdk-5.0.0-beta1-linux.tar.gz

ls -l
```

**注意：** 如果下载的是 beta 版本，如 `jindosdk-5.0.0-beta1-linux.tar.gz`，需要按如下方式重命名。

```bash
mv jindosdk-5.0.0-beta1-linux.tar.gz jindosdk-5.0.0-linux.tar.gz
```
...
-rw-r----- 1 emr-user emr-user xxxxxxxxx May 01 00:00 jindosdk-5.0.0-linux.tar.gz
```
...
以将新版集群中的 JindoSDK 从 4.6.3 版本升级到 5.0.0(beta1) 版本为例:

```bash
./apply_all.sh 5.0.0 4.6.3
```
...
以将新版集群中的 JindoSDK 升级到 5.0.0 版本，linux x86平台为例:
...
wget https://jindodata-binary.oss-cn-shanghai.aliyuncs.com/release/5.0.0/jindosdk-5.0.0-linux.tar.gz
...
```bash
cp jindosdk-5.0.0-beta1-linux.tar.gz jindosdk-5.0.0-linux.tar.gz
```
...
-rw-r----- 1 hadoop hadoop xxxxxxxxx May 01 00:00 jindosdk-5.0.0-linux.tar.gz
...
以将新版集群中的 JindoSDK 从 4.6.3 版本升级到 5.0.0 版本为例:

```bash
bash bootstrap_jindosdk.sh -gen 5.0.0 4.6.3
```
